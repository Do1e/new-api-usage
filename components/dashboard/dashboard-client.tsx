...
const getDefaultFilters = (defaultRecentDays: number): FilterState => {
  if (defaultRecentDays <= 0) {
    return {
      startTime: null,
      endTime: null,
      user: null,
  ...
  const startTime = Math.floor(startOfToday.getTime() / 1000) - defaultRecentDays * 86400;
...
const DashboardClient = ({ defaultRecentDays }: DashboardClientProps) => {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [filters, setFilters] = useState<FilterState>(() =>
    getDefaultFilters(defaultRecentDays),
  );
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <LogsTable filters={filters} refreshKey={refreshKey} />
      </div>
    </div>
  );
};
